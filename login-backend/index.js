...
app.get("/api/tours", async (req, res) => {
    try {
        ...
        res.status(500).send({ message: "Error fetching tours" });
    }
});
...
app.get("/api/tours/:id", async (req, res) => {
    try {
        ...
        res.status(500).send({ message: "Error fetching tour details" });
    }
});

// API Route to save the booking form
app.post("/api/book-tour", async (req, res) => {
    try {
        const newBooking = new Booking(req.body);
        await newBooking.save();
        res.status(201).send({ message: "Booking submitted successfully! We will contact you shortly." });
    } catch (error) {
